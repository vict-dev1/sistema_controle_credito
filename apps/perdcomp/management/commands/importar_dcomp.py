...
class Command(BaseCommand):
    ...
    def converter_booleano(self,value):
        if value in ['Sim', 'sim', 'Yes', 'yes', '1', True]:
            return True
        elif value in ['Não', 'nao', 'No', 'no', '0', False]:
            return False
    ...
    def handle(self,*args,**options):
        ...
        if not textos:
            self.stdout.write("Nenhum texto extraido dos PDFs. Verifique os arquivos no diretório.")
            return
        ...
        for texto in textos:
            try:
                ...
                perdcomp_retificador = self.converter_booleano(perdcomp_retificador)
                ...
                credito_oriundo_de_acao_judicial = self.converter_booleano(credito_oriundo_de_acao_judicial)
                ...
                pessoa_juridica_extinta_por_liquidacao_voluntaria = self.converter_booleano(pessoa_juridica_extinta_por_liquidacao_voluntaria)
                ...
                informado_em_processo_admistrativo_anterior = self.converter_booleano(informado_em_processo_admistrativo_anterior)
                ...
                informado_em_outro_perdcomp = self.converter_booleano(informado_em_outro_perdcomp)
                ...
                situacao_especial_do_titular_credito = self.converter_booleano(situacao_especial_do_titular_credito)
                ...
                selic_acumulada = self.converter_booleano(selic_acumulada)
                ...
                credito_sucedido = self.converter_booleano(credito_sucedido)
